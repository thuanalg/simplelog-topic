...
template <> struct std::formatter<my_type> : std::formatter<std::string> {
	auto format(my_type my, format_context &ctx) const -> decltype(ctx.out())
	{
...
#include <chrono>
#include <spdlog/spdlog.h>
#include <spdlog/sinks/basic_file_sink.h> // Để sử dụng file sink
...
void
log_messages(int thread_id, int num_messages)
{
	const char *p = 0;
	for (int i = 0; i < num_messages; ++i) {
		// logger->info("Thread {} logging message {}", thread_id, i);
		__FILLE__(p);
		logger->info("[{}:{}:{}] [Threadid {}] test log: {}", p, __FUNCTION__, __LINE__, GetCurrentThreadId(), i);
		// logger->info("[{}:{}:{}] [Threadid {}] test log: {}", p, __FUNCTION__, __LINE__,
		//    pthread_self(), i);
	}
}

void
run_performance_test(int num_threads, int num_messages)
{
	...
	for (int i = 0; i < num_threads; ++i) {
		threads.push_back(std::thread(log_messages, i, num_messages));
	}
	...
	std::cout << "Test completed in " << duration << " seconds with " << num_threads * num_messages
		  << " messages logged." << std::endl;

	double throughput = (num_threads * num_messages) / static_cast<double>(duration);
...
main()
{
	// Cấu hình logger để ghi vào tệp log.txt
	logger = spdlog::basic_logger_mt("A", "log_thuannt.txt");

	// Cấu hình mức độ log (info, debug, warning...)
	spdlog::set_level(spdlog::level::info);

	// Thực hiện thử nghiệm với 10 luồng và 1000 thông điệp mỗi luồng
	int num_threads = 10;
	int num_messages = 1000 * 1000;

	// Chạy thử nghiệm đo hiệu suất
	run_performance_test(num_threads, num_messages);
