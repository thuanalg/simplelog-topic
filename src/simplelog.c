...
#define SPL_FCLOSE(__fp__, __n)                                                \
	{                                                                      \
		if (__fp__) {                                                  \
			...
			} else { /*spl_console_log("Close FILE 0x%p DONE.",    \
				    (__fp__));*/                               \
				;                                              \
				(__fp__) = 0;                                  \
				;                                              \
...
#define SPL_pthread_mutex_destroy(__obj, __err)                                \
	{                                                                      \
		...
		if ((__err))                                                   \
			spl_console_log(                                       \
			    "pthread_mutex_destroy errcode: %d. %s\n",         \
			    (__err), (__err) ? "FALIED" : "DONE");             \
	}
#define SPL_pthread_mutex_lock(__obj, __err)                                   \
	{                                                                      \
		...
		if ((__err))                                                   \
			spl_console_log(                                       \
			    "pthread_mutex_lock errcode: %d. %s\n", (__err),   \
			    (__err) ? "FALIED" : "DONE");                      \
	}
#define SPL_pthread_mutex_unlock(__obj, __err)                                 \
	{                                                                      \
		...
		if ((__err))                                                   \
			spl_console_log(                                       \
			    "pthread_mutex_unlock errcode: %d. %s\n", (__err), \
			    (__err) ? "FALIED" : "DONE");                      \
	}
#endif
/*+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+*/
...
/*+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+*/
static const char *__splog_pathfolder[] = {SPLOG_PATHFOLDR, SPLOG_LEVEL,
    SPLOG_BUFF_SIZE, SPL_MAX_SZ_MSG, SPLOG_ROT_SIZE, SPLOG_TOPIC, SPLOG_NCPU,
    SPLOG_TRIGGER, SPLOG_END_CFG, 0};
...
static int
spl_create_memory(
    void **output, char *shared_key, int size_shared, char isCreating);
static int
spl_del_memory();
static int
spl_calculate_size();
...
spl_local_time_now(spl_local_time_st *stt)
{
	...
	do {
		...
		stt->nn = (unsigned int)lt.wMilliseconds * SPL_MILLION +
			  counter.QuadPart % SPL_MILLION;
#else
		time_t t = time(0);
...
spl_init_log_parse(char *buff, char *key, char *isEnd)
{
	...
	do {
		if (strcmp(key, SPLOG_PATHFOLDR) == 0) {
			...
			snprintf(
			    __simple_log_static__.folder, 1024, "%s", buff);
			break;
		}
...
spl_init_log_ext(SPL_INPUT_ARG *input)
{
	int ret = 0;
	do {
		memcpy(__simple_log_static__.id_name, input->id_name,
		    SPL_IDD_NAME);
	...
	if (name) {
		if (__simple_log_static__.id_name[0]) {
			snprintf(name, SPL_FNAME_LEN, "%.4d-%.2d-%.2d-%s",
			    ...
			    (char *)__simple_log_static__.id_name);
		} else {
			snprintf(name, SPL_FNAME_LEN,
			    "%.4d-%.2d-%.2d-simplelog", lt.year + YEAR_PADDING,
			    (int)lt.month + MONTH_PADDING, (int)lt.day);
		}
	}
	return ret;
...
#endif
{
	...
	//spl_create_memory((void**)&only_buf, "thread_buff_123", (t->buff_size
	* t->ncpu), 1);
	*/
	...
	if (t->arr_topic) {
		spl_malloc(
		    t->n_topic * sizeof(char *), src_topic_thrd_buf, char **);
		for (i = 0; i < t->n_topic; ++i) {
			char *p = (char *)t->arr_topic[i].buf;
			spl_malloc(t->ncpu * sizeof(char *),
	...
	do {
		...
		*/
		...
		while (1) {
			...
			do {
				ret = spl_gen_file(
				    t, &sz, t->file_limit_size, &(t->index));
				if (ret) {
					spl_console_log(
					    "--spl_gen_file, ret: %d --\n",
					    ret);
				...
				for (i = 0; i < t->ncpu; ++i) {
					...
					/* //do { */
					if (MYCASTGEN(main_src_thrd_buf[i])
						->pl > 0) {
						memcpy(only_cast->data +
							   only_cast->pl,
						    MYCASTGEN(
							main_src_thrd_buf[i])
							->data,
							...
							main_src_thrd_buf[i])
							->pl);
						only_cast->pl += MYCASTGEN(
						    main_src_thrd_buf[i])
								     ->pl;
						MYCASTGEN(main_src_thrd_buf[i])
						    ->pl = 0;
					}
					/* //} while (0); */
...
spl_simple_log_thread(SIMPLE_LOG_ST *t)
{
	...
	do {
		...
		DWORD thread_id = 0;
		hd = CreateThread(
		    NULL, 0, spl_written_thread_routine, t, 0, &thread_id);
		if (!hd) {
...
/*+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+*/
char *
spl_fmt_now_ext(char *fmtt, int len, int lv, const char *filename,
    const char *funcname, int line, unsigned short *r, int *outlen)
{
	char *p = fmtt;
	int ret = 0;
	...
	*outlen += snprintf(
	    fmtt + n, SPL_RL_BUF - n, "[%s:%s:%d] ", filename, funcname, line);
	if (*outlen > len) {
...
spl_fmmt_now(char *fmtt, int len)
{
	...
	do {
		...
		n = snprintf(buff, 20, SPL_FMT_DATE_ADDING,
		...
		n = snprintf(buff1, 20, SPL_FMT_HOUR_ADDING, stt.hour,
		    stt.minute, stt.sec);
		n = snprintf(
		    fmtt, len, SPL_FMT_MILL_ADDING, buff, buff1, (int)stt.nn);
...
spl_gen_file(SIMPLE_LOG_ST *t, int *sz, int limit, int *index)
{
	...
	do {
		...
		if (!t->fp) {
			...
			if (ret) {
				spl_console_log(
				    "spl_folder_sup: ret: %d.\n", ret);
				break;
			}
			do {
				int err = 0;
				int cszize = 0;
				snprintf(path, 1024, SPL_FILE_NAME_FMT,
				    t->folder, yearmonth, fmt_file_name,
				    *index);
				snprintf(t->path_template, 1024,
				    SPL_FILE_NAME_FMT_TOPIC, t->folder,
				    yearmonth, fmt_file_name);
		...
		snprintf(path, 1024, SPL_FILE_NAME_FMT, t->folder, yearmonth,
		    fmt_file_name, *index);
		snprintf(t->path_template, 1024, SPL_FILE_NAME_FMT_TOPIC,
		    t->folder, yearmonth, fmt_file_name);
...
spl_folder_sup(char *folder, spl_local_time_st *lctime, char *year_month)
{
	...
	do {
		...
		snprintf(
		    path, 1024, "%s/%.4u", folder, lctime->year + YEAR_PADDING);
		result = CreateDirectoryA(path, 0);
		if (!result) {
			DWORD xerr = GetLastError();
			if (xerr != ERROR_ALREADY_EXISTS) {
		...
		snprintf(path, 1024, "%s/%.4d/%.2d", folder,
...
spl_fclose_err(int terr, void *ffp)
{
	...
	do {
#ifndef UNIX_LINUX
		spl_console_log("ffp: %p, terr: %d, GetLastError: 0x%x,", ffp,
		    terr, (int)GetLastError());
#else
		char buf[64];
		...
		spl_console_log("ffp: %p,terr: %d, errno: %d, strerror_r: %s",
...
#endif
{
	...
	*outhd = hThread;
	...
	if (ret) {
		...
		spl_console_log("pthread_create: ret: %d, errno: %d, text: %s.",
...
#define spl_shm_unlink(__name__, __err__)                                      \
	{                                                                      \
		...
		if (__err__) {                                                 \
			spl_console_log("shm_unlink: err: %d, errno: %d, "     \
					"text: %s, name: %s.",                 \
...
spl_del_memory()
{
	...
	do {
		...
		if (!isWell) {
			spl_console_log(
			    "UnmapViewOfFile error: %d", (int)GetLastError());
			ret = SPL_LOG_SHM_WIN_UNMAP;
		}
		isWell = (int)CloseHandle((HANDLE)t->hd);
		if (!isWell) {
			spl_console_log("SPL_LOG_WIN_SHM_CLOSE, err: %d",
			    (int)GetLastError());
		...
		if (ret) {
			...
			spl_console_log("shm_unlink: err: %d, errno: %d, text: "
					"%s, name: %s.",
	...
	do {
		...
		if (isCreating) {
			hMapFile = CreateFileMappingA(
			    ...
			    size_shared, /*   // maximum object size (low-order
					    DWORD) */
			    shared_key); /*    // name of mapping object */

			if (!hMapFile) {
				spl_console_log(
				    "Cannot create SHM. error: %d\n",
				    (int)GetLastError());
				ret = 1;
				break;
			}
		...
		p = (char *)MapViewOfFile(
		    hMapFile, FILE_MAP_ALL_ACCESS, 0, 0, size_shared);
		if (!p) {
			ret = 3;
		...
		int hMapFile = 0;
		int err = 0;
		hMapFile = shm_open(shared_key, SPL_LOG_UNIX_CREATE_MODE,
		    SPL_LOG_UNIX__SHARED_MODE);
		if (hMapFile > 0) {
			break;
		}
		hMapFile = shm_open(shared_key, SPL_LOG_UNIX_OPEN_MODE,
		    SPL_LOG_UNIX__SHARED_MODE);
		if (hMapFile < 1) {
			spl_console_log(
			    "SPL_LOG_SHM_UNIX_OPEN option creating");
		...
		p = (char *)mmap(0, size_shared, SPL_LOG_UNIX_PROT_FLAGS,
		    MAP_SHARED, hMapFile, 0);
		if (p == MAP_FAILED || p == 0) {
...
/*+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+*/
/* (SIMPLE_LOG_ST*)&__simple_log_static__, volatile long, pthread_spinlock_t,
 * pthread_mutex_t */
	...
	do {
		if (!t->arr_mtx) {
			...
			if (!t->arr_mtx) {
				...
				spl_console_log(
				    "spl_malloc: SPL_LOG_ARR_MTX_NULL.");
				break;
			}
		}
#ifndef UNIX_LINUX
...
#error "not yet implemented."
		...
		*/
		...
		/*semsize: sem size.*/
		/*+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-*/
		spl_console_log("buf size: %d, mtxsize: %d, semsize: %d",
		    (int)k, (int)mtxsize, (int)semsize);
		n = k + mtxsize + semsize;
		t->map_mem_size = n;
		/*+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-*/
		/* int spl_create_memory(void** output, char* shared_key, int
		 * size_shared, char isCreating) */
		if (t->isProcessMode) {
			spl_create_memory(
			    (void **)&buff, t->shared_key, n, t->is_master);
		} else {
		...
		if (!buff) {
			...
			spl_console_log(
			    "spl_malloc: SPL_LOG_BUFF_MALLOC_ERROR.");
			break;
		}
		...
		if (t->mtx_rw) {
			pthread_spinlock_t *mtx =
			    (pthread_spinlock_t *)t->mtx_rw;
			if (t->isProcessMode) {
				int err = 0;
				err = pthread_spin_init(
				    mtx, PTHREAD_PROCESS_SHARED);
				if (err) {
		...
		if (t->sem_rwfile && t->sem_off) {
			int err = 0;
			err = sem_init(
			    (sem_t *)t->sem_rwfile, (int)t->isProcessMode, 0);
			if (err) {
				ret = SPL_LOG_SEM_INIT_UNIX;
				spl_console_log(
				    "sem_init, errno: %d, errno_text: %s.",
				    errno, strerror(errno));
				break;
			}
			err = sem_init(
			    (sem_t *)t->sem_off, (int)t->isProcessMode, 0);
			if (err) {
				ret = SPL_LOG_SEM_INIT_UNIX;
				spl_console_log(
				    "sem_init, errno: %d, errno_text: %s.",
...
spl_win32_sync_create()
{
	...
	do {
		...
		if (t->isProcessMode) {
			HANDLE hd = 0;
			int i = 0;
			snprintf(nameobj, SPL_SHARED_NAME_LEN, "%s_%s",
			    SPL_MTX_NAME_OFF, t->shared_key);
			hd = CreateMutexA(0, 0, nameobj);
			if (!hd) {
				ret = SPL_LOG_MTX_WIN32_CREATED_ERROR;
				spl_console_log("CreateMutexA, errno: %d.",
				    (int)GetLastError());
				break;
			}
			t->mtx_rw = hd;
			...
			for (i = 0; i < t->ncpu; ++i) {
				snprintf(nameobj, SPL_SHARED_NAME_LEN,
				    "%s_%s_%0.2d", SPL_MTX_NAME_OFF,
				    t->shared_key, i);
				hd = CreateMutexA(0, 0, nameobj);
				if (!hd) {
					ret = SPL_LOG_MTX_WIN32_CREATED_ERROR;
					spl_console_log(
					    "CreateMutexA, errno: %d.",
					    (int)GetLastError());
					break;
				}
				t->arr_mtx[i] = hd;
		...
		if (t->isProcessMode) {
			HANDLE hd = 0;
			snprintf(nameobj, SPL_SHARED_NAME_LEN, "%s_%s",
			    SPL_SEM_NAME_RW, t->shared_key);
			hd = CreateSemaphoreA(0, 0, 1, nameobj);
			if (!hd) {
				spl_console_log("CreateSemaphoreA, errno: %d.",
				    (int)GetLastError());
	...
	do {
		...
		if (t->isProcessMode | 1) {
			...
			if (sem_close((sem_t *)t->sem_rwfile) == -1) {
				spl_console_log(
				    "sem_close, errno: %d, errno_text: %s.",
				    errno, strerror(errno));
			...
			if (sem_unlink(nameobj) == -1) {
				spl_console_log(
				    "sem_unlink, errno: %d, errno_text: %s.",
				    errno, strerror(errno));
	...
	do {
		...
		if (t->isProcessMode | 1) {
			...
			snprintf(nameobj, SPL_SHARED_NAME_LEN, "%s_%s",
			    SPL_SEM_NAME_RW, t->shared_key);
			do {
				hd = sem_open(nameobj, SPL_LOG_UNIX_CREATE_MODE,
				    SPL_LOG_UNIX__SHARED_MODE, 1);
				if (hd == SEM_FAILED) {
					spl_console_log("sem_open, errno: %d, "
							"errno_text: %s.",
					    errno, strerror(errno));
	...
	do {
		if (shared) {
			...
			if (err) {
				...
				spl_console_log("pthread_mutexattr_init, "
						"errno: %d, errno_text: %s.",
				    errno, strerror(errno));
				break;
			}
			...
			if (err) {
				...
				spl_console_log("pthread_mutexattr_setpshared, "
						"errno: %d, errno_text: %s.",
				    errno, strerror(errno));
				break;
			}
			...
			if (err) {
				...
				spl_console_log("pthread_mutex_init, errno: "
						"%d, errno_text: %s.",
				    errno, strerror(errno));
				break;
			}
...
spl_init_segments()
{
	...
	p = (char *)t->buf;
	if (!t->range) {
		t->range = t->buff_size - (sizeof(generic_dta_st) +
					      t->max_sz_msg + SPL_RL_BUF);
...
spl_allocate_topics()
{
	...
	do {
		...
		for (i = 0; i < t->n_topic; ++i) {
			p1 = strstr(p0, ",");
			if (!p1) {
				snprintf(t->arr_topic[i].topic, SPL_TOPIC_SIZE,
				    "%s", p0);
				continue;
			}
			n = (int)(p1 - p0);
			if (n > 0) {
				snprintf(t->arr_topic[i].topic,
				    SPL_MIN_AB(SPL_TOPIC_SIZE, n + 1), "%s",
				    p0);
			}
			p1++;
			p0 = p1;
