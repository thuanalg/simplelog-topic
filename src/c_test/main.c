...
main(int argc, char *argv[])
{
	...
	for (i = 1; i < argc; ++i) {
		if (strstr(argv[i], TNUMBEER_OF_THREADS) == argv[i]) {
			ret = sscanf(
			    argv[i], TNUMBEER_OF_THREADS "%d", &num_threads);
			continue;
		}
...
#ifndef UNIX_LINUX
	snprintf(
	    cfgpath, 1024, "C:/z/simplelog-topic/win64/Debug/simplelog.cfg");
#else
...
dotest()
{
	...
	for (i = 0; i < num_threads; ++i) {
		hpThread[i] = CreateThread(
		    NULL, 0, win32_thread_routine, 0, 0, (dwpThreadId + i));
	}
	// https://learn.microsoft.com/en-us/windows/win32/api/synchapi/nf-synchapi-waitformultipleobjects
	// https://learn.microsoft.com/en-us/windows/win32/sync/waiting-for-multiple-objects
	dwEvent =
	    WaitForMultipleObjects(num_threads, // number of objects in array
...
#define spllogsys(__level__, __fmt__, ...)                                     \
	spllogtopic(__level__, 0, __fmt__, ##__VA_ARGS__);
#define splloglib(__level__, __fmt__, ...)                                     \
	spllogtopic(__level__, 1, __fmt__, ##__VA_ARGS__);
#define spllogexe(__level__, __fmt__, ...)                                     \
	spllogtopic(__level__, 2, __fmt__, ##__VA_ARGS__);
#define spllognaxyax(__level__, __fmt__, ...)                                  \
	spllogtopic(__level__, 3, __fmt__, ##__VA_ARGS__);
#define spllogsksgn(__level__, __fmt__, ...)                                   \
	spllogtopic(__level__, 4, __fmt__, ##__VA_ARGS__);
...
#endif // !UNIX_LINUX
	int count = 0;
	/*#define SPL_TEST_FMT			"test log test log test log:
	 * %d"*/
#define SPL_TEST_FMT "My test log : %d"
	if (topicindex < 1) {
		...
		while (count < loop_count) {
			spllogtopic(
			    SPL_LOG_INFO, topicindex - 1, SPL_TEST_FMT, count);
			++count;
		}
	}
	return 0;
