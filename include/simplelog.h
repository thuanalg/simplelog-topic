 ...
 *		<2025-Feb-16>
 * Decription:
 *		The (only) main header file to export 4 APIs: [spl_init_log_ext,
 *spllog, spllogtopic, spl_finish_log].
 */
/*+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+*/
...
#define spl_console_log(___fmttt___, ...)                                      \
	{                                                                      \
		...
		fprintf(stdout,                                                \
		    ...
		    (int)__LINE__, spl_get_threadid(), ##__VA_ARGS__);         \
	}
#else
#define spl_console_log(___fmttt___, ...)                                      \
	{                                                                      \
		...
		fprintf(stdout, __c11fmt__.c_str(), (char *)buf, (char *)pfn,  \
		    (char *)__FUNCTION__, (int)__LINE__, spl_get_threadid(),   \
		    ##__VA_ARGS__);                                            \
	}
#endif
...
#define SPLKEYBUF(__t__, __i__)                                                \
	((generic_dta_st *)((char *)__t__->buf + (__t__->buff_size * __i__)))
#define __spl_log_buf_level__(__lv__, ___fmttt___, ...)                         \
	{                                                                       \
		...
		if (__t__->llevel <= (__lv__) && ___fmttt___[0]) {              \
			...
			;                                                       \
			__FILLE__(__pfn__);                                     \
			__pprefmt__ = spl_fmt_now_ext(__tnow__, SPL_RL_BUF,     \
			...
			{                                                       \
				do {                                            \
					...
					;                                       \
					if (__t__->range >                      \
					    SPLKEYBUF(__t__, __r__)->pl) {      \
						;                               \
						memcpy(SPLKEYBUF(__t__, __r__)  \
							       ->data +         \
							   SPLKEYBUF(           \
							       __t__, __r__)    \
							       ->pl,            \
...
#define STSPLOGBUFTOPIC_RANGE(__t__, __i__, __r__)                             \
	((generic_dta_st *)((char *)STSPLOGBUFTOPIC(__t__, __i__) +            \
	...
	{                                                                      \
		...
		if (__t__->llevel <= (__lv__) && ___fmttt___[0] &&             \
			...
			do {                                                   \
				...
				;                                              \
				;                                              \
				if (__t__->range > STSPLOGBUFTOPIC_RANGE(      \
						       __t__, __tpp__, __r__)  \
						       ->pl) {                 \
					;                                      \
					memcpy(STSPLOGBUFTOPIC_RANGE(          \
						   __t__, __tpp__, __r__)      \
						       ->data +                \
						   STSPLOGBUFTOPIC_RANGE(      \
						       __t__, __tpp__, __r__)  \
						       ->pl,                   \
					    __pprefmt__, __outlen__);          \
					;                                      \
 ...
 * Sample:		spllogtopic(SPL_LOG_INFO, 0, "Hello spllog: %llu",
 * time(0));
 */
